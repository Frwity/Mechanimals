    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Enemy") && transform.parent.CompareTag("Player"))
        {
            ...
            enemy.TakeDamage(player.upperBodyChara.GetSpecialDamage(), 3,
                (transform.position.x < enemy.transform.position.x ? player.knockBackDirection : new Vector2(-player.knockBackDirection.x, player.knockBackDirection.y)) * player.upperBodyChara.GetSpecialKnockbackForce());
        }
    }
}
